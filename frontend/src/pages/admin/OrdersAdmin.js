...
function OrdersAdmin() {
    ...
    const handleUpdateStatus = async (id, status) => {
        try {
            ...
            setMsg({ type: "error", text: "Lỗi khi cập nhật!" });
        }
    };
                    ...
                    <div style={{ background: "#fff", borderRadius: "10px", boxShadow: "0 2px 8px rgba(0,0,0,0.08)", overflow: "auto" }}>
                        <table style={{ width: "100%", borderCollapse: "collapse" }}>
                            <thead style={{ background: "#f5a623" }}>
                                <tr>
                                    {["Mã đơn", "Khách hàng", "SĐT", "Địa chỉ", "Tổng tiền", "Ngày đặt", "Trạng thái", "Thao tác"].map(h => (
                            ...
                            <tbody>
                                {filtered.length === 0 ? (
                                    <tr><td colSpan={8} style={{ textAlign: "center", padding: "24px", color: "#888" }}>Không có đơn hàng</td></tr>
                                ) : filtered.map((o, i) => {
                                    ...
                                    return (
                                        <tr key={o.id} style={{ background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                                            ...
                                            <td style={{ ...tdStyle, maxWidth: "150px" }}>
                                                <span title={o.address} style={{ display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                                    {o.address}
                                                </span>
                                            </td>
                                            <td style={tdStyle}>{Number(o.total).toLocaleString()}đ</td>
                                            ...
                                            <td style={tdStyle}>
                                                ...
                                                {" "}
                                                <button onClick={() => handleDelete(o.id)} style={btnStyle("#dc3545", "sm")}>Xóa</button>
                                            </td>
                                        </tr>
                                    );
                                })}
            ...
            {showModal && selected && (
                <div style={overlayStyle}>
                    <div style={modalStyle}>
                        ...
                        <p><b>Tổng tiền:</b> {Number(selected.total).toLocaleString()}đ</p>
                            ...
                            {Object.entries(STATUS_LABELS).map(([k, v]) => (
                                <button
                                    ...
                                    style={{
                                        ...
                                        color: selected.status === k ? "#fff" : "#333",
                                        fontWeight: 600
                                    }}
                                >
                                    {v.label}
